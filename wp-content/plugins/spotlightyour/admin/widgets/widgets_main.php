<?php
include_once(DDB_WIDGET_FOLDER_PATH.'widgets_sidebar.php'); // all register sidebar widget
$widget_array = array();
...
$widget_array = apply_filters('DDBWP_widgets_listing_filter',$widget_array);
if($widget_array)
{
	foreach($widget_array as $key=>$val)
	{
		...
		{
			$filename = $val;
			if(file_exists(DDB_WIDGET_FOLDER_PATH.$filename))
			{
				include_once(DDB_WIDGET_FOLDER_PATH.$filename);
			}
		}
	}
}
