...
$_SESSION['count'] = 0;
// =============================== Location Map Widget ======================================
if(!class_exists('DDBWP_locationmap'))
{
	class DDBWP_locationmap extends WP_Widget {
		function DDBWP_locationmap() {
		//Constructor
			$widget_ops = array('classname' => 'widget location_map', 'description' => apply_filters('DDBWP_googlemap_widget_desc_filter','Enter your location OR latitude and longitude to show address on map'));		
			$this->WP_Widget('widget_location_map', apply_filters('DDBWP_googlemap_widget_title_filter','T &rarr; Google Map Location'), $widget_ops);
		}
	...
	<a class="large_map b_getdirection" target="_blank" href="http://maps.google.com/maps?f=q&amp;source=s_q&amp;hl=en&amp;geocode=&amp;q=<?php echo $address;?>&amp;sll=<?php echo $geo_latitude;?>,<?php echo $geo_longitude;?>&amp;ie=UTF8&amp;hq=&ll=<?php echo $geo_latitude;?>,<?php echo $geo_longitude;?>&amp;spn=0.368483,0.891953&amp;z=11&iwloc=A"><?php _e('View Large Map','ddb_wp');?></a>
	</div>
	<?php }?>
    <?php if($address){?>
    <span class="get_direction"><a href="http://maps.google.com/maps?f=d&amp;dirflg=d&amp;saddr=<?php echo $address;?>" target="_blank"><?php echo apply_filters('DDBWP_googlemap_widget_get_direction_filter',__('Get Direction on Map &raquo;','ddb_wp'));?></a></span>
    <?php }?>
	</div>
		<?php
	...
	<p><label for="<?php echo $this->get_field_id('address'); ?>"><?php _e('Address <small>(eg: 230 Vine Street And locations throughout Old City, Philadelphia, PA 19106)</small>','ddb_wp');?> : 
	...
	<p><label for="<?php  echo $this->get_field_id('address_latitude'); ?>"><?php _e('Address Latitude <small>(eg: 39.955823048131286)</small>','ddb_wp');?> : <input class="widefat" id="<?php  echo $this->get_field_id('address_latitude'); ?>" name="<?php echo $this->get_field_name('address_latitude'); ?>" type="text" value="<?php echo attribute_escape($address_latitude); ?>" /></label></p>
	
	<p><label for="<?php  echo $this->get_field_id('address_longitude'); ?>"><?php _e('Address Longitude <small>(eg: -75.14408111572266)</small>','ddb_wp');?> : 
	...
	<p><label for="<?php echo $this->get_field_id('map_width'); ?>"><?php _e('Map Width in pixcels <small>(eg: 200)</small>','ddb_wp');?> : 
	...
	<p><label for="<?php echo $this->get_field_id('map_height'); ?>"><?php _e('Map Height in pixcels <small>(eg: 200)</small>','ddb_wp');?> : <input type="text" class="widefat" rows="6" cols="20" id="<?php echo $this->get_field_id('map_height'); ?>" name="<?php echo $this->get_field_name('map_height'); ?>" value="<?php echo attribute_escape($map_height); ?>"></label></p>
	
	
	<p>
	<label for="<?php echo $this->get_field_id('scale'); ?>"><?php _e('Map Zooming Factor','ddb_wp');?> : 
	...
	<p>
	<label for="<?php echo $this->get_field_id('map_type'); ?>"><?php _e('Select Map Type','ddb_wp');?> : 
	<select id="<?php echo $this->get_field_id('map_type'); ?>" name="<?php echo $this->get_field_name('map_type'); ?>">
	<option value="ROADMAP" <?php if(attribute_escape($map_type)=='ROADMAP'){echo 'selected="selected"';}?> ><?php _e('Road Map','ddb_wp');?></option>
	<option value="SATELLITE" <?php if(attribute_escape($map_type)=='SATELLITE'){echo 'selected="selected"';}?>><?php _e('Satellite Map','ddb_wp');?></option>
	<option value="TERRAIN" <?php if(attribute_escape($map_type)=='TERRAIN'){echo 'selected="selected"';}?>><?php _e('Terrain Map','ddb_wp');?></option>
	</select>
	</label>
	</p>
	<?php
