<?php
// ===============================  Latest Posts - widget ======================================
if(!class_exists('DDBWP_latest_posts_with_images')){
	class DDBWP_latest_posts_with_images extends WP_Widget {
	
		function DDBWP_latest_posts_with_images() {
		//Constructor
		global $thumb_url;
			$widget_ops = array('classname' => 'widget special', 'description' => apply_filters('DDBWP_latestpostwimg_widget_desc_filter','Post with image & date') );
			$this->WP_Widget('latest_posts_with_images',apply_filters('DDBWP_latestpostwimg_widget_title_filter','T &rarr; Post with image & date'), $widget_ops);
		}
		...
		// prints the widget
         ...
         <?php 
            ...
            <?php }elseif($post_images = bdw_get_images($post->ID)){ ?>
             <a  class="post_img" href="<?php the_permalink(); ?>">
             <img  src="<?php echo DDBWP_thumbimage_filter(get_post_meta($post->ID,'file_name',true),'&amp;w=50&amp;h=50&amp;zc=1&amp;q=80',1);?>" alt="<?php the_title(); ?>" title="<?php the_title(); ?>" width="50" height="50"  /> </a>
            <?php
            }?>
					
            <h4> <a class="widget-title" href="<?php the_permalink(); ?>">
                  <?php the_title(); ?>
                  </a> <br />  <span class="post_author">by <?php the_author_posts_link(); ?> at <?php echo $post->post_date; ?>  / <?php comments_popup_link(__('No Comments','ddb_wp'), __('1 Comment','ddb_wp'), __('% Comments','ddb_wp'), '', __('Comments Closed','ddb_wp')); ?> </span></h4> 
                  
                  <p> <?php echo bm_better_excerpt(175, ''); ?> <a href="<?php the_permalink(); ?>"> <?php _e('more...','ddb_wp');?> </a></p> 
        </li>
	<?php endforeach; ?>
				</ul>
	...
	<p>
	  <label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('Number of posts:','ddb_wp');?>
    ...
    <p>
<label for="<?php echo $this->get_field_id('post_type'); ?>"><?php _e('Post Type:','ddb_wp')?>
...
<?php }}?>
	...
	<p>
	  <label for="<?php echo $this->get_field_id('category'); ?>"><?php _e('Cities (<code>IDs</code> separated by commas):','ddb_wp');?>
