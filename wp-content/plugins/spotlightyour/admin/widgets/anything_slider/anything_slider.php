...
include_once(DDB_WIDGET_FOLDER_PATH.'anything_slider/custom_post_type_slider/custom_post_type_slider_lang.php');
include_once(DDB_WIDGET_FOLDER_PATH.'anything_slider/custom_post_type_slider/custom_post_type_slider.php');
if(!class_exists('DDBWP_anything_slider')){
class DDBWP_anything_slider extends WP_Widget {
	function DDBWP_anything_slider() {
		$widget_ops = array('classname' => 'widget anything_slider','description' =>  apply_filters('DDBWP_anything_slider_widget_desc_filter','Any thing Slider Widget'));
		$this->WP_Widget('anything_slider', apply_filters('DDBWP_anything_slider_widget_title_filter','T &rarr; Any thing Slider'), $widget_ops);
	}
...
<script type="text/javascript" src="<?php echo DDB_WIDGET_JS_FOLDER_URL; ?>jquery_slider2.js"></script>
<script src="<?php echo DDB_WIDGET_JS_FOLDER_URL; ?>jquery_slider2plus.js" type="text/javascript" charset="utf-8"></script>
...
$today_special = apply_filters('DDBWP_anything_slider_widget_postsobj_filter',get_posts('numberposts='.$numberpost.$pquery.''));
...
<li>
<?php
$custom_design = apply_filters('DDBWP_anything_slider_widget_content_filter',$post);
if(is_object($custom_design))
{
	echo DDBWP_anything_slider_class_fun($post);
}else
{
	echo $custom_design;	
}
...
<p>
  <label for="<?php echo $this->get_field_id('numberpost'); ?>"><?php _e('Number of posts:','ddb_wp')?>
...
<p>
  <label for="<?php echo $this->get_field_id('delay'); ?>"><?php _e('Slider delay time :','ddb_wp')?>
...
<p>
  <label for="<?php echo $this->get_field_id('animationTime'); ?>"><?php _e('Slider animation time :','ddb_wp')?>
...
<p>
  <label for="<?php echo $this->get_field_id('height'); ?>"><?php _e('Slider Banner Height (i.e.400):','ddb_wp')?>
...
register_widget('DDBWP_anything_slider');
}

function DDBWP_anything_slider_class_fun($post)
{
