			...
			<li>
				<div class="posts_deal">							
					...
					if(get_post_meta($deals_res->ID,'file_name',true) != "") { ?>
						<img src="<?php echo DDBWP_thumbimage_filter(get_post_meta($deals_res->ID,'file_name',true),'&amp;w=165&amp;h=180&amp;zc=1&amp;q=80');?>" alt=""/>
			<?php 	} else{ ?>
			...
			<?php 	} ?></a>
				...
				<?php 	if(get_post_meta($deals_res->ID,'coupon_end_date_time',true) != '') {
						...
						<strong><?php echo DEAL_CONTENT_TEXT; ?> : </strong> <br />
						 ...
						 <?php echo $deals_res->post_content;?>
						<ul class="deal_li">
				...
				<?php 		if($deals_res->post_author == $current_user->data->ID) { ?>
								<li><span class="field"><?php echo DEAL_CODE_TEXT; ?> </span><span>:  <?php _e(get_post_meta($deals_res->ID,'coupon_code',true),'ddb_wp');?></span></li>
				<?php 		} ?>
							<li><span class="field"><?php echo DEAL_POST_TEXT; ?> </span><span>: <?php _e($deals_res->post_date,'ddb_wp'); ?></span></li>							
							<li><span class="field"><?php echo DEAL_OWNER_TEXT; ?> </span><span>: <?php _e(get_post_meta($deals_res->ID,'owner_name',true),'ddb_wp'); ?></span></li>
							<li><span class="field"><?php echo DEAL_NUM_OF_TEXT; ?> </span><span>: <?php _e(get_post_meta($deals_res->ID,'no_of_coupon',true),'ddb_wp') ?></span></li>
							<li><span class="field"><?php echo DEAL_PRICE_TEXT; ?> </span><span>: <?php _e(get_post_currency_sym($deals_res->ID).get_post_meta($deals_res->ID,'our_price',true),'ddb_wp'); ?></span></li>
							<li><span class="field"><?php echo DEAL_CPRICE_TEXT; ?> </span><span>: <?php _e(get_post_currency_sym($deals_res->ID).get_post_meta($deals_res->ID,'current_price',true),'ddb_wp');?></span></li>
						</ul>
						<p class="deallistinglinks">
							<span class="link"><?php echo DEAL_WEBLINK_TEXT; ?> : <a href="<?php _e(get_post_meta($deals_res->ID,'coupon_website',true),'ddb_wp') ;?>" title="<?php _e($transactiondeal->coupon_website,'ddb_wp');?>" target="_blank"><?php _e(get_post_meta($deals_res->ID,'coupon_website',true),'tempalatic');?></a></span></p>
						</div>	
					</div>
				</li>							
			</ul>					
...
<?php	if($transaction) {
		...
		if($transaction_obj->user_id == $current_user->ID) {  ?>
			<tr class="list_row">
				...
				<td class="row1"><?php echo get_post_currency_sym($_REQUEST['transid']).$transaction_obj->payable_amt; ?></td>
				...
				<td colspan="6">	
					<table border="" class="sub_table">
						...
						<tr>
							...
							<td><span><?php echo PAID_AMOUNT; ?></span>: <?php echo get_post_currency_sym($_REQUEST['transid']).$transaction_obj->payable_amt; ?></td>
						</tr>
						<tr>
