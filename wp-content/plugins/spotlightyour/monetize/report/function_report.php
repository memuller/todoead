...
define ("PLUGIN_URL_REPORT", DDB_PUGIN_URL.'/monetize/'.PLUGIN_DIR_REPORT.'/');
define ("PLUGIN_STYLEURL_REPORT", DDB_PUGIN_URL.'/monetize/');
global $wpdb,$table_prefix;
$transection_db_table_name = $table_prefix . "deal_transaction";
add_action('DDBWP_admin_menu', 'report_add_admin_menu');
function report_add_admin_menu()
{
	add_submenu_page('ddb_wp_wp_admin_menu', __("Manage Orders",'ddb_wp'), __("Manage Order",'ddb_wp'), DDBWP_ACCESS_USER, 'report', 'manage_report');
}
function manage_report()
{
	DDB_checkLicence();
	include_once(DDB_MODULES_FOLDER_PATH . 'report/manage_report_list.php');
}
...
add_action('ddb_wp_function','ifetch_status');
function upcoming_transaction_add_dashboard_widgets() {
	if(is_super_admin($current_user->ID)) {
	wp_add_dashboard_widget('ddb_wp_upcoming_transaction', 'Recent Deal', 'upcoming_transaction_dashboard_widget');

	global $wp_meta_boxes;

	$normal_dashboard = $wp_meta_boxes['dashboard']['normal']['core'];

	$example_widget_backup = array('ddb_wp_upcoming_appointments' => $normal_dashboard['ddb_wp_upcoming_transaction']);
	unset($normal_dashboard['ddb_wp_upcoming_transaction']);
...
function upcoming_transaction_dashboard_widget() {
	...
	if($live_posts){
		echo '<link href="'.DDB_PUGIN_URL.'/monetize/admin.css" rel="stylesheet" type="text/css" />';
