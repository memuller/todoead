...
register_post_type(	"$custom_post_type", 
				array(	'label' 			=> CUSTOM_MENU_TITLE,
						...
						'menu_icon' 		=> ''/*DDB_PUGIN_URL.'/images/favicon.ico'*/,
						'hierarchical' 		=> false,
...
add_filter( 'manage_edit-seller_columns', 'ddb_wp_edit_seller_columns' ) ;

function ddb_wp_edit_seller_columns( $columns ) {
...
add_action( 'manage_seller_posts_custom_column', 'ddb_wp_manage_seller_columns', 10, 2 );

function ddb_wp_manage_seller_columns( $column, $post_id ) {
	...
	switch( $column ) {
		...
		case 'coupon_start_date_time' :
			...
			if ( $coupon_start_date_time != '' ) {
				echo date('d M, Y H:i:s',$coupon_start_date_time);
			}
		...
		case 'coupon_end_date_time' :
			...
			if ( !empty( $coupon_end_date_time ) ) {
				echo date('d M, Y H:i:s',$coupon_end_date_time);
			}
		...
		case 'post_category' :
			...
			if(DDBWP_is_show_post_category()){
...
add_filter( 'manage_edit-seller_sortable_columns', 'ddb_wp_seller_sortable_columns' );

function ddb_wp_seller_sortable_columns( $columns ) {
