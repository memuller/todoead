...
<p><?php _e('Custom user-info fields help you gather required information from sellers to display on their profile page. They will fill their information in the fields you set here.','ddb_wp');?></p>
<?php
if($_REQUEST['msg']=='delsuccess')
{
	$message = __('Field deleted successfully.','ddb_wp');	
}
?>
<?php if($message){?>
...
<table width="100%"  class="widefat post" >
  <thead>
    <tr>
      <th><?php _e('Title','ddb_wp');?></th>
      <th align="center"><?php _e('Type','ddb_wp');?></th>
      <th align="center"><?php _e('Activated?','ddb_wp');?></th>
      <th><?php _e('Display Order','ddb_wp');?></th>
...
if($post_meta_info){
	...
	?>
	 ...
	 <a href="javascript:void(0);showdetail('<?php echo $post_meta_info_obj->cid;?>');"><?php _e('Detail','ddb_wp');?></a> | <a href="<?php echo site_url();?>/wp-admin/admin.php?page=custom_usermeta&cf=<?php echo $post_meta_info_obj->cid;?>#option_add_custom_usermeta"><?php _e('Edit','ddb_wp');?></a> <?php if($post_meta_info_obj->is_delete=='0'){?> | <a href="javascript:void(0);" onclick="return confirm_delete('<?php echo $post_meta_info_obj->cid;?>');"><?php _e('Delete','ddb_wp');?></a><?php }?>
      ...
      <tr>
        <td><?php _e('Title','ddb_wp')?> :  <strong><?php echo $post_meta_info_obj->site_title;?><strong></td>
	  ...
	  <tr>
		...
		<td><?php _e('Is Require','ddb_wp')?> :  <strong><?php echo $post_meta_info_obj->is_require;?><strong></td>
        
		<td><?php _e('Activated?','ddb_wp')?> :  <strong><?php if($post_meta_info_obj->is_active) _e('Yes','ddb_wp'); else _e('No','ddb_wp');?><strong></td>
	  </tr>
		<tr>
			<td><?php _e('Display On Registration?','ddb_wp')?> :  <strong><?php if($post_meta_info_obj->show_on_listing) _e('Yes','ddb_wp'); else _e('No','ddb_wp');?><strong></td>
			<td colspan="2"><?php _e('Display On Profile?','ddb_wp')?> :  <strong><?php if($post_meta_info_obj->show_on_detail) _e('Yes','ddb_wp'); else _e('No','ddb_wp');?><strong></td>
		</tr>
       <tr>        
        
      	
         <td colspan="3"><?php _e('Use at front end','ddb_wp')?> :  <strong><?php if($post_meta_info_obj->is_delete=='0'){echo 'get_usermeta($user_id,"'.$post_meta_info_obj->htmlvar_name.'")';}elseif($post_meta_info_obj->is_delete=='1'){_e('Theme Default Field','ddb_wp');}elseif($post_meta_info_obj->ctype=='head'){_e('Heading','ddb_wp');}?><strong></td>
      </tr>
      </table>
      </td>
      </tr>
