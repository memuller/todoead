...
if($_REQUEST['cf'])
{
	 _e('Edit custom user-info field','ddb_wp');	
}else
{
	 _e('Add a custom user-info field','ddb_wp');
}
?></h3>
<?php
if($_REQUEST['msg']=='success')
{
	$message = __('User-info field saved successfully.','ddb_wp');	
}
?>
<?php if($message){?>
...
</script><p><?php _e('You can add a new custom user-info field here. Users will be able to add / edit their information on their profile page.','ddb_wp');?></p>
...
<br />    <p><?php _e('Select the type of this custom field.','ddb_wp');?></p>
  ...
  <td align="left"><input type="text" name="option_values" id="option_values" value="<?php echo $post_val->option_values;?>" size="50" />
   <br />    <p><?php _e('Specify multiple option values seperated by a comma.','ddb_wp');?></p>
  </td>
  </tr>
  ...
  <tr>
  <td ><strong><?php _e('Field name: ','ddb_wp');?></strong></td>
...
<br />    <p><?php _e('Custom field description which will appear in the front-end as well as the backend.','ddb_wp');?></p>
 ...
 <tr id="admin_label_id"  <?php if($post_val->ctype=='head'){?> style="display:none;" <?php }else{?>  style="display:block;" <?php }?>>
  ...
  <option value="0" <?php if($post_val->is_active=='0'){ echo 'selected="selected"';}?>><?php _e('No','ddb_wp');?></option>
      ...
      <br />    <span><?php _e('This setting activates/de-activates the custom field in the front-end and the back-end.','ddb_wp');?></span>
 </td>
  </tr>
  ...
  <option value="0" <?php if($post_val->is_require=='0'){ echo 'selected="selected"';}?>><?php _e('No','ddb_wp');?></option>
      ...
      <br />    <span><?php _e('Specify whether or not this field is required to be filled in compulsarily by users.','ddb_wp');?></span>
 </td>
  </tr>
  <tr id="show_on_listing_id"  <?php if($post_val->ctype=='head'){?> style="display:none;" <?php }else{?> style="display:block;" <?php }?>>
  <td ><strong><?php _e('Show on registration page?: ','ddb_wp');?></strong></td>
  <td align="left">
  ...
  <option value="0" <?php if($post_val->show_on_listing=='0'){ echo 'selected="selected"';}?>><?php _e('No','ddb_wp');?></option>
      ...
      <br />    <span><?php _e('Specify whether or not this field be shown on the &lsquo;Registration page&rsquo;.','ddb_wp');?></span>
  </td>
  </tr>
  <tr id="show_on_detail_id"  <?php if($post_val->ctype=='head'){?> style="display:none;" <?php }else{?> style="display:block;" <?php }?>>
  <td ><strong><?php _e('Show on Profile page ? : ','ddb_wp');?></strong></td>
  <td align="left">
  ...
  <option value="0" <?php if($post_val->show_on_detail=='0'){ echo 'selected="selected"';}?>><?php _e('No','ddb_wp');?></option>
      ...
      <br />    <span><?php _e('Specify whether or not this field be shown on the &lsquo;Profile page&rsquo;.','ddb_wp');?></span>
  </td>
  </tr>
    <tr>
  <td >&nbsp;</td>
  <td align="left"><input type="submit" class="button" name="save" id="save" value="<?php _e('Save','ddb_wp');?>" /> <a href="<?php echo site_url();?>/wp-admin/admin.php?page=custom_usermeta"><input type="button" name="cancel" value="<?php _e('Cancel','ddb_wp');?>" class="button" /></a></td>
  </tr>
</table>
</form>
<script type="text/javascript">
