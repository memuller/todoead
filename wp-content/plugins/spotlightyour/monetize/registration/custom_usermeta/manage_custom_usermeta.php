<?php
//Custom Settings
if(!function_exists('DDBWP_get_user_custom_meta_array')){
function DDBWP_get_user_custom_meta_array()
{
	$pt_metaboxes = array();
    ...
    if($pt_metabox['type'] == 'text' OR $pt_metabox['type'] == 'select' OR $pt_metabox['type'] == 'radio' OR $pt_metabox['type'] == 'checkbox' OR $pt_metabox['type'] == 'textarea' OR $pt_metabox['type'] == 'upload' OR $pt_metabox['type'] == 'date' OR $pt_metabox['type'] == 'multicheckbox' OR $pt_metabox['type'] == 'texteditor')
			...
			elseif ($pt_metabox['type'] == 'multicheckbox'){
			 ...
			 elseif ($pt_metabox['type'] == 'date'){
				...
				$output .= "\t".'<div>';
                $output .= "\t\t".'<br/><p><strong><label for="'.$pt_id.'">'.$pt_metabox['label'].'</label></strong></p>'."\n";
                $output .= "\t\t".'<p><input size="40" class="pt_input_text" type="text" value="'.$pt_metaboxvalue.'" name="ddbwpthemes_'.$pt_metabox["name"].'" /><img src="'.DDB_PUGIN_URL.'/images/cal.gif" alt="Calendar"  onclick="displayCalendar(document.post.ddbwpthemes_'.$pt_metabox["name"].',\'yyyy-mm-dd\',this)" style="cursor: pointer;" align="absmiddle" border="0" /></p>'."\n";
                $output .= "\t\t".'<p><span style="font-size:11px">'.$pt_metabox['desc'].'</span></p>'."\n";
                $output .= "\t".'</div>'."\n";
            ...
            elseif ($pt_metabox['type'] == 'checkbox'){
				...
				$output .= "\t".'<div>';
                $output .= "\t\t".'<br/><p><strong><label for="'.$pt_id.'">'.$pt_metabox['label'].'</label></strong></p>'."\n";
                $output .= "\t\t".'<p><input type="checkbox" '.$checked.' class="pt_input_checkbox"  id="'.$pt_id.'" value="1" name="ddbwpthemes_'. $pt_metabox["name"] .'" /></p>'."\n";
                $output .= "\t\t".'<p><span style="font-size:11px">'.$pt_metabox['desc'].'</span></p>'."\n";
                $output .= "\t".'</div>'."\n";
            }
      ...
      <div class="element">';
                $output .= "\t\t".'<br/><p><strong><label for="'.$pt_id.'">'.$pt_metabox['label'].'</label></strong></p>'."\n";
                $output .= '<input type="text" class="'.$up_class.'"  id="ddbwpthemes_'. $pt_metabox["name"] .'" name="ddbwpthemes_'. $pt_metabox["name"] .'" value="'.$pt_metaboxvalue.'"/> <input id="upload_'.$pt_id.'" class="upload_button" type="button" value="'.__('Upload','ddb_wp').'" rel="'.$pt_id.'" /><div class="screenshot" id="ddbwpthemes_'. $pt_metabox["name"] .'_image">';
			...
			if($pt_metabox['type'] == 'geo_map'){
                ...
                $output .= "\t".'<div>';
                $output .= "\t\t".'<br/><p><strong><label for="geo_address">'.__('Geo Address','ddb_wp').'</label></strong></p>'."\n";
                $output .= "\t\t".'<p><input size="100" class="pt_input_text" type="text" value="'.$geo_address.'" name="ddbwpthemes_geo_address" id="geo_address"/></p>'."\n";
                $output .= "\t\t".'<p><span style="font-size:11px">'.__('Please enter address. eg. : 230 Vine Street And locations throughout Old City, Philadelphia, PA 19106','ddb_wp').'</span></p>'."\n";
                $output .= "\t".'</div>'."\n";
				
				$geo_latitude = get_post_meta($post->ID,'geo_latitude',true);
                $output .= "\t".'<div>';
                $output .= "\t\t".'<br/><p><strong><label for="geo_latitude">'.__('Geo Latitude','ddb_wp').'</label></strong></p>'."\n";
                $output .= "\t\t".'<p><input size="100" class="pt_input_text" type="text" value="'.$geo_latitude.'" name="ddbwpthemes_geo_latitude" id="geo_latitude"/></p>'."\n";
                $output .= "\t\t".'<p><span style="font-size:11px">'.__('Enter Geo Latitude. eg. : 38.8225909761771','ddb_wp').'</span></p>'."\n";
                $output .= "\t".'</div>'."\n";
				
				$geo_longitude = get_post_meta($post->ID,'geo_longitude',true);
                $output .= "\t".'<div>';
                $output .= "\t\t".'<br/><p><strong><label for="geo_longitude">'.__('Geo Longitude','ddb_wp').'</label></strong></p>'."\n";
                ...
                $output .= "\t\t".'<p><span style="font-size:11px">'.__('Enter Geo Longitude. eg. : 12.65625','ddb_wp').'</span></p>'."\n";
                $output .= "\t".'</div>'."\n";
