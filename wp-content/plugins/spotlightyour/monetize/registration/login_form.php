...
case 'retrievepassword' :
	if ( $http_post ) {
		...
		if ( !is_wp_error($errors) ) {
			global $General;
			wp_redirect($General->get_url_login(site_url()).'/?dwtype=login&action=login&checkemail=confirm');
			exit();
		}
	}
...
case 'rp' :
	...
	global $General;
	if ( ! is_wp_error($errors) ) {
		wp_redirect($General->get_url_login(site_url()).'/?dwtype=login&action=login&checkemail=newpass');
		exit();
	}

	wp_redirect($General->get_url_login(site_url()).'/?dwtype=login&action=lostpassword&error=invalidkey');
	exit();
...
case 'register' :
	if ( !get_option('users_can_register') ) {
		wp_redirect(site_url().'/?dwtype=login&registration=disabled');
		exit();
	}
	...
	if ( $http_post ) {
		...
		}else
		{
			if ( !is_wp_error($errors) ) {
				global $General;
				wp_redirect($General->get_url_login(site_url()).'/?dwtype=login&action=login&checkemail=registered');
				exit();
			}	
		}
...
?>
<div class="login_content"> <?php echo stripslashes(get_option('ddbwpthemes_logoin_page_content'));?> </div>
<div class="login_form_box">
  <form name="loginform" id="loginform" action="<?php echo get_settings('home').'/index.php?dwtype=login&amp;dwtype1='.$_REQUEST['dwtype']; ?>" method="post" >
  ...
  <div id="lostpassword_form" <?php if($display_style == '') { echo $display_style; } else { echo 'style="display:none;"';} ?> >
    ...
    <form name="lostpasswordform" id="lostpasswordform" action="<?php echo site_url().'/?dwtype=login&amp;action=lostpassword'; ?>" method="post">
      <div class="form_row clearfix">
