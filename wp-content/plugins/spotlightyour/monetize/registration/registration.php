...
function retrieve_password() {
	...
	$message .= '<p>You can login to : <a href="'.site_url().'/?dwtype=login' . "\">Login</a> or the URL is :  ".site_url()."/?dwtype=login</p>";
...
function register_new_user($user_login, $user_email) {
	...
	// Check the username
	if ( $user_login == '' )
		$errors->add('empty_username', __('ERROR: Please enter a username.','ddb_wp'));
	elseif ( !validate_username( $user_login ) ) {
		$errors->add('invalid_username', __('<strong>ERROR</strong>: This username is invalid.  Please enter a valid username.','ddb_wp'));
		$user_login = '';
	...
	if ( !$user_id ) {
		...
		return $errors;
			...
			$redirect_to = wp_redirect(site_url().'/?reg=1&dwtype=login');
	}
...
<p>[#site_name#]</p>";
		...
		$store_login = '<a href="'.site_url().'/?dwtype=login">'.site_url().'/?dwtype=login</a>';
		...
		if(get_option('pttthemes_send_mail') == 'Enable' ) {	
			DDBWP_sendEmail($fromEmail,$fromEmailName,$user_email,$userName,$subject,$client_message,$extra='');
		}///To clidne email
		$redirect_to = wp_redirect(site_url().'/?reg=1&dwtype=login');
...
case 'logout' :
	...
	//$redirect_to = site_url().'/?dwtype=login&loggedout=true';
...
case 'retrievepassword' :
	if ( $http_post ) {
		...
		if ( !is_wp_error($errors) ) {
			wp_redirect(site_url().'/?dwtype=login&page1=sign_in&checkemail=confirm');
			exit();
		}else
		{
			wp_redirect(site_url().'/?dwtype=login&page1=sign_in&emsg=fw');
			exit();
		}
	}
	if ( isset($_GET['error']) && 'invalidkey' == $_GET['error'] ) $errors->add('invalidkey', __('Sorry, that key does not appear to be valid.','ddb_wp'));
...
case 'rp' :
	...
	if ( ! is_wp_error($errors) ) {
		wp_redirect(site_url().'/?dwtype=login&action=login&checkemail=newpass');
		exit();
	}

	wp_redirect(site_url().'/?dwtype=login&action=lostpassword&page1=sign_in&error=invalidkey');
	exit();
...
case 'register' :
	...
	if ( !get_option('users_can_register') ) {
		wp_redirect(site_url().'?dwtype=login&page1=sign_up&emsg=regnewusr');
		exit();
	}
	if ( $http_post ) {
		...
		if ( !is_wp_error($errors) ) 
		{
			...
			if($_REQUEST['reg_redirect_link']=='')
			{
				...
				$_REQUEST['reg_redirect_link']= site_url("/?dwtype=login&reg=1");
			}else{
			$redirect_to = site_url("/?dwtype=login&reg=1");
			}
...
default:
	...
	if(is_wp_error($user))
	{
		if(strstr($_SERVER['HTTP_REFERER'],'dwtype=submition') && $_POST['log']!='' && $_POST['pwd']!='')
		{
	...
	if(!strstr($_SERVER['HTTP_REFERER'],'dwtype=submition'))
	{  
		if($_REQUEST['dwtype1'] != '') {
	
		$redirect_to = site_url()."/?dwtype=".$_REQUEST['dwtype1'];
	...
	if(($_POST['log'] && $errors) || ($_POST['log']=='' && $_REQUEST['testcookie']))
	{
		...
		{
			wp_redirect(site_url().'?dwtype=login&page1=sign_in&logemsg=1');
		}
		exit;
	}
break;
  ...
  <div class="entry">
    <div  id="post_<?php the_ID(); ?>">
      <div class="post-meta">
        ...
        <?php echo DDBWP_page_title_filter(SIGN_IN_PAGE_TITLE); //page tilte filter?>
...
?>
        <div class="login_form">
          <?php 
	include (DDBWP_REGISTRATION_FOLDER . "login_form.php");
 	?>
        </div>
        <?php
}
elseif($_REQUEST['page']=='login' && $_REQUEST['page1']=='sign_up')
{
	?>
        <div class="registration_form">
          <?php include (DDBWP_REGISTRATION_FOLDER . "registration_form.php");?>
        </div>
        <?php
...
?>
        <div class="login_form_l">
          <?php include (DDBWP_REGISTRATION_FOLDER . "login_form.php");?>
        </div>
...
try{document.getElementById('user_login').focus();}catch(e){}
      ...
      <script type="text/javascript" src="<?php echo DDBWP_REGISTRATION_URI; ?>registration_validation.js"></script>
      <?php
