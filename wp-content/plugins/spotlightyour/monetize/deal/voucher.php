...
a.i_print { background:url(<?php echo DDB_PUGIN_URL;?>/images/i_print.png) no-repeat left top; padding-left:22px; padding-top:1px; text-decoration:none;float:right;color:#000000;font-size:14px; }	
...
<div class="wrapper">
	<div class="box">
		<div class="logo">
			...
			$transaction_childdeal = $wpdb->get_row("select * from $transaction_table where trans_id = '".$_REQUEST['transaction_id']."'"); 
			?>
		</div>
		<?php 
		global $current_user;
		...
		if($current_user->ID){ 
			if($current_user->ID == $transaction_childdeal->user_id || $current_user->ID == $transaction_childdeal->author_id){				
		?>
		...
		<p class="deal_paid"><?php echo PAID_AMOUNT;?> <span class="amount"><?php echo get_post_currency_sym($transaction_childdeal->post_id).number_format($transaction_childdeal->payable_amt,2);?></span></p>
...
<?php include_once (DDB_PUGIN_PATH . '/library/map/preview_map.php');?>
...
<?php }
		...
		echo '<p align="center"><em>You are not authorized to view this Coupon</em></p>';
		exit;
	 } }else {
	echo '<script>location.href="'.site_url().'/?dwtype=login&redirect_to='.urlencode(site_url()."/?dwtype=voucher&transaction_id=".$_REQUEST['transaction_id']).'"</script>';
		exit;
}?>
	</div>
</div>
