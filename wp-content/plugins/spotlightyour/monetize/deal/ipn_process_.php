	...
	if (eregi('VERIFIED', $info)) 
	{
		...
		if($deal_details[1]=='4' || $deal_details[1]=='5')
		{
			...
			if($coupon_address!="")
			{
				...
				--------------------------------------------------\r",'ddb_wp'),$postid,$item_name,$coup,$coupon_address,$last_trans_id,$txn_id,$payment_status,$payment_type,$payment_date,$txn_type);
			}
			else
			{
				$transaction_details = sprintf(__("--------------------------------------------------\r
				...
				Deal Coupon: %s \r	--------------------------------------------------\r
				Trans ID: %s\r Paypal Trans ID: %s\r Status: %s\r	Type: %s\r	Date: %s\r	Method: %s\r
				--------------------------------------------------\r",'ddb_wp'),$postid,$item_name,$coup,$last_trans_id,$txn_id,$payment_status,$payment_type,$payment_date,$txn_type);
			}
		}
		else
		{
			...
			Trans ID: %s\r Paypal Trans ID: %s\r Status: %s\r	Type: %s\r	Date: %s\r	Method: %s\r
			--------------------------------------------------\r",'ddb_wp'),$postid,$item_name,$coup,$post_title_link,$last_trans_id,$txn_id,$payment_status,$payment_type,$payment_date,$txn_type);
		}
		...
		--------------------------------------------------\r",'ddb_wp'),$last_trans_id,$deal_details[2],$deal_details[3],$deal_details[4],$Billing_address);
		...
		if($deal_details[1]=='4' || $deal_details[1]=='5')
		{
			...
			--------------------------------------------------\r",'ddb_wp'),$deal_details[11],$Sipping_address);
		}
		// Start User Details With Deal
		...
		DDBWP_sendEmail($to_seller_email,$to_seller_name,$fromEmail,$fromEmailName,$email_admin_subject,$email_admin_content,$extra='');///To admin email
		...
		DDBWP_sendEmail($deal_details[3],$deal_details[2],$fromEmail,$fromEmailName,$email_user_subject,$email_user_content,$extra='');///To admin email
		...
		DDBWP_sendEmail($fromEmail,$fromEmailName,$deal_details[3],$deal_details[2],$email_admin_subject,$email_admin_content,$extra='');///To admin email
