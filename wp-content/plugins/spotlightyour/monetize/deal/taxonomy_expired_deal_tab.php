...
<script type="text/javascript" src="<?php echo DDB_PUGIN_URL;?>/js/timer.js"></script>
 ...
 <?php 
	...
	if(strstr($main_url,'?')) {
	if(strstr($main_url,'dwtype=taxonomy_all_deal_tab') == false){
		$all_deal_querystr = "&amp;dwtype=taxonomy_all_deal_tab";
	} if( strstr($main_url,'dwtype=taxonomy_live_deal_tab') == false){
		$live_deal_querystr = "&amp;dwtype=taxonomy_live_deal_tab";
	} if(strstr($main_url,'dwtype=taxonomy_expired_deal_tab') == false) {
