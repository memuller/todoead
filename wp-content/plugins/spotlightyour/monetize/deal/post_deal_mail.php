...
if(isset($_POST['update_deal']) || $_SESSION['deal_id'] != ""){
	...
	if($_SESSION != '')
	{
		...
		wp_redirect(site_url("/?dwtype=success_deal&edit_success=yes"));
	}
...
if(isset($_REQUEST['save_deal']) && $_REQUEST['save_deal'] != '' && $_SESSION['deal_id'] == "")
{
	...
	if($coupon_address!="")	{
			...
			",'ddb_wp'),$deal_title,$deal_desc,$no_of_coupon,get_post_currency_sym($lastid).$current_price,get_post_currency_sym($lastid).$our_price,$current_price,$coupon_address);
	} else	{
			$transaction_details = sprintf(__("
			<p><strong>".DEAL_DETAILS_TEXT."</strong> \r</p>
			<p><strong>".DEAL_TITLE_TEXT.": </strong>%s \r	</p>
			...
			<p><strong>".DEAL_CPRICE_TEXT.": </strong>%s \r	</p>
			<p><strong>".DEAL_YOUR_PRICE_TEXT.": </strong>%s \r	</p>
			",'ddb_wp'),$deal_title,$deal_desc,$no_of_coupon,$coupon_type,get_post_currency_sym($lastid).$current_price,get_post_currency_sym($lastid).$our_price);
	}
	...
	<p><strong>".EMAIL.":</strong> %s \r	</p><p><strong>Password :</strong> %s \r</p>",'ddb_wp'),$user_fname,$user_email,$password);
	...
	/*-------Fetch Email details --------*/
		...
		if(get_option('pttthemes_send_mail') == 'Enable' || get_option('pttthemes_send_mail') == '') {	
			DDBWP_sendEmail($user_email,$user_fname,$fromEmail,$fromEmailName,$subject,$email_seller_content,$extra='');
		...
		if ( $current_user->data->ID == '' && !isset($current_user->data->ID) ) {
			...
			if(get_option('pttthemes_send_mail') == 'Enable' || get_option('pttthemes_send_mail') == '') {	
				DDBWP_sendEmail($fromEmail,$fromEmailName,$user_login,$user_fname,$subject,$client_message,$extra='');
			}///To clidne email
			//////REGISTRATION EMAIL END////////
		}
		if(isset($_REQUEST['save_deal']) && $_REQUEST['save_deal'] != '' && $_SESSION['deal_id'] == "")
		{
		wp_redirect(site_url("/?dwtype=success_deal"));
		}
		unset($_SESSION['deal_info']);
}
?>