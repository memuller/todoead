				...
				if (strcmp ($res, "VERIFIED") == 0) 
				{
					...
					if($transifo['deal_type']=='4' || $transifo['deal_type']=='5')
					{
						...
						if($coupon_address!="")
						{
							...
							<p>Trans ID: %s\r</p> <p>Paypal Trans ID: %s\r</p> <p>Status: %s\r</p>	<p>Type: %s\r</p>	<p>Date: %s\r</p>	<p>Method: %s\r</p>",'ddb_wp'),$postid,$item_name,$coup,$coupon_address,$last_trans_id,$txn_id,$payment_status,$payment_type,$payment_date,$txn_type);
						}
						else
						{
							$transaction_details = sprintf(__("
							...
							<p>Deal Title: %s \r</p>
							<p>Deal Coupon: %s \r	</p>
							<p>Trans ID: %s\r</p> <p>Paypal Trans ID: %s\r</p> <p>Status: %s\r</p>	<p>Type: %s\r</p>	<p>Date: %s\r</p>	<p>Method: %s\r</p>",'ddb_wp'),$postid,$item_name,$coup,$last_trans_id,$txn_id,$payment_status,$payment_type,$payment_date,$txn_type);
						}
					}
					else
					{
						...
						<p>Trans ID: %s\r</p> <p>Paypal Trans ID: %s\r</p> <p>Status: %s\r</p>	<p>Type: %s\r</p>	<p>Date: %s\r</p>	<p>Method: %s\r</p>",'ddb_wp'),$postid,$item_name,$coup,$post_title_link,$last_trans_id,$txn_id,$payment_status,$payment_type,$payment_date,$txn_type);
					}
					...
					<p>Billing Name: %s\r</p> <p>Billing Address: %s\r </p>",'ddb_wp'),$last_trans_id,$transifo['user_name'],$transifo['pay_email'],$transifo['billing_name'],$transifo['billing_add']);
					
					if($transifo['deal_type']=='4' || $transifo['deal_type']=='5')
					{
						$users_details = sprintf(__("
					   <p> Shipping  Details\r	</p>
						<p>Shipping Name: %s\r</p> <p>Shipping Address: %s\r </p>",'ddb_wp'),$transifo['shipping_name'],$transifo['shipping_add']);
					}
					// Start User Details With Deal
					...
					if(get_option('ddbwpthemes_deal_pay_notify_seller')=='Yes' || get_option('ddbwpthemes_deal_pay_notify_seller')==''){
					...
					{
						$email_seller_subject =  __("Acknowledgment for your Deal",'ddb_wp');
						
					}
					if(!$email_seller_content)
					{
						$email_seller_content=__('<p>Dear [#to_name#],</p><p>[#deal_details#]</p><br><p>[#user_details#]</p><br><p>Thanks!</p><p>[#site_name#]</p>','ddb_wp');
					}
					...
					DDBWP_sendEmail($fromEmail,$fromEmailName,$to_seller_email,$to_seller_name,$email_seller_subject,$email_seller_content,$extra='');///To admin email
					}
					// End Payment Success To Seller Email
					
					
					// Start  Payment Success To USer Email
					if(get_option('ddbwpthemes_deal_pay_notify_user')=='Yes' || get_option('ddbwpthemes_deal_pay_notify_user')==''){
					...
					{
						$email_user_subject =  __("Acknowledgment for your Payment",'ddb_wp');
						
					}
					if(!$email_user_content)
					{
						$email_user_content=__('<p>Dear [#to_name#],</p><p>[#deal_details#]</p><br><p>We hope you enjoy. Thanks!</p><p>[#site_name#]</p>','ddb_wp');
					}
					...
					DDBWP_sendEmail($fromEmail,$fromEmailName,$transifo['pay_email'],$transifo['user_name'],$email_user_subject,$email_user_content,$extra='');///To admin email
					}
					...
					if(get_option('ddbwpthemes_deal_pay_notify_admin')=='Yes' || get_option('ddbwpthemes_deal_pay_notify_admin')==''){
					...
					{
						$email_admin_subject =  __("Payment received successfully",'ddb_wp');
						
					}
					if(!$email_admin_content)
					{
						$email_admin_content=__('<p>Dear [#to_name#],</p><p>[#deal_details#]</p><br><p>[#user_details#]</p><br><p>. Thanks!</p><p>[#site_name#]</p>','ddb_wp');
					}
					$search_array = array('[#to_name#]','[#deal_details#]','[#user_details#]','[#site_name#]');
					...
					DDBWP_sendEmail($transifo['pay_email'],$transifo['user_name'],$fromEmail,$fromEmailName,$email_admin_subject,$email_admin_content,$extra='');///To admin email
					}
