<?php $deal_targetpage = "?dwtype=taxonomy_all_deal_tab";
	...
	//echo $dealcnt."  limit $strtlimit,$recordsperpage ";
...
<?php 			} ?></a>
			...
			<div class="content_right content_right_inner">
	           <?php if(get_option('ddbwpthemes_listing_author') != 'No') {?>        
                ...
                <div class="grid_price"><span class="strike_rate"><?php echo OUR_PRICES;?><s><?php echo get_post_currency_sym($deal_data->ID);?><?php echo $current_price;?></s></span> <span class="rate"><?php echo OFFER_PRICE;?><?php echo get_post_currency_sym($deal_data->ID);?><?php echo $our_price;?></span> </div>
<?php 			if(date("Y-m-d H:i:s")>= $enddate1  && isset($enddate1) && $enddate1 != '') {
					if(get_post_meta($post->ID,'is_expired',true)=='0' || get_post_meta($post->ID,'no_of_coupon',true)==$sellsqlinfo) {
	...
	<?php 			}
					...
					if(get_post_meta($post->ID,'coupon_end_date_time',true)) { ?> 
                        ...
                        <div class="deal_time_box">
							...
							<span class="strike_rate"><?php echo get_post_currency_sym($deal_data->ID);?><?php echo $current_price;?></span> 
                            <span class="rate"><?php echo get_post_currency_sym($deal_data->ID);?><?php echo $our_price;?></span> 
                            </div>
