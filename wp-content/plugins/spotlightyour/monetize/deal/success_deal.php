...
<p class="error"><?php _e(SUCCESS_EDIT_DEAL,'ddb_wp');?></p>
<?php }else { 
// Start All Transection Details With Deal
	...
	if($coupon_address!="")	{
		$deal_detail = sprintf(__("
			...
			",'ddb_wp'),$deal_title,$deal_desc,$no_of_coupon,get_post_currency_sym().$current_price,get_post_currency_sym().$our_price,$current_price,$coupon_address);
	} else	{
		$deal_detail = sprintf(__("
			<h5 class='title'>".DEAL_DETAILS_TEXT."</h5>
			<ul>
			...
			",'ddb_wp'),$deal_title,$deal_desc,$no_of_coupon,$coupon_type,get_currency_sym().$current_price,get_currency_sym().$our_price);
		}
