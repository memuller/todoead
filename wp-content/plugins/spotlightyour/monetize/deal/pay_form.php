...
?>
<script type="text/javascript" src="<?php echo DDB_PUGIN_URL;?>/monetize/deal/buy_deal_validation.js"></script>
...
<div  class="content content_full" >
	...
	?>
    <div class="post-meta">
				<h1 class="entry-title"><?php echo BUY_DEAL_TITLE;?></h1>
			</div>
    ...
    <div class="sect">       
		...
		{
			_e('<p class="error_msg">'.BUY_DEAL_LMT.'</p>','ddb_wp');
		}else{
		?>
        <?php
		...
		?>
			<div class="form_row clearfix">
				...
				<?php echo get_post_currency_sym($post->ID); echo $price; ?>
			</div>
			<p><?php _e(BUY_DEAL_DESC,'ddb_wp');?></p>
            <div id="post_<?php the_ID(); ?>">
				<div class="post-content">
		<?php 		if($_REQUEST['msg']=='nopaymethod'){?>
						<p class="error"><?php _e('Please Select Payment Method','ddb_wp');?></p>
		<?php 		}?>
...
<!-- BOF Shipping Form -->
					<div class="post_deal_buy_now"> 
						...
						<form id="dealpaynow_frm" name="dealpaynow_frm" action="<?php echo site_url().'/?dwtype=dealpaynow';?>" method="post">
		...
		<?php					if(file_exists(DDB_PUGIN_PATH.'/library/includes/payment/'.$paymentInfo['key'].'/'.$paymentInfo['key'].'.php')) { ?>
		<?php						include_once(DDB_PUGIN_PATH.'/library/includes/payment/'.$paymentInfo['key'].'/'.$paymentInfo['key'].'.php');	} ?> 						</li>
		<?php						}
								}
							}else {	?>
		...
		<?php			}
						if(get_option('pttthemes_captcha')=='Enable' && file_exists(ABSPATH.'wp-content/plugins/wp-recaptcha/recaptchalib.php') && plugin_is_active('wp-recaptcha')){
							...
							if($_REQUEST['emsg']=='captch'){echo '<span class="message_error2" id="category_span">'.__('Please enter valid text as you see in captcha','ddb_wp').'</span>';}
							echo '</div>';
						} ?> 
