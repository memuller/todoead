...
if($_REQUEST['cf'])
{
	 _e('Edit custom field','ddb_wp');	
}else
{
	 _e('Add a new custom field','ddb_wp');
}
?></h3>
<?php
if($_REQUEST['msg']=='success')
{
	$message = __('Saved successfully.','ddb_wp');	
}
?>
<?php if($message){?>
...
function chk_field_form()
{
...

	{
		...
		alert("<?php _e('Please enter frontend title','ddb_wp');?>");
...

	{
		...
		alert("<?php _e('Please Enter HTML Variable Name','ddb_wp');?>");
...
</script><p><?php _e('You can add a new custom field here. While submitting a post or a deal, sellers will be required to fill it.','ddb_wp');?></p>
...
<tr>
  <td width="30%"><strong><?php _e('Show in post-type: ','ddb_wp');?></strong></td>
...
<br />    <p><?php _e('Specify whether this custom field is shown while adding/editing a new Post or while adding/editing a new Deal.','ddb_wp');?></p>
  </td>
  </tr>
<tr>
  <td width="30%"><strong><?php _e('Field type: ','ddb_wp');?></strong></td>
  <td align="left">
   <select name="ctype" id="ctype" onchange="show_option_add(this.value)" <?php if($post_val->is_delete=='1'){?> disabled="disabled" <?php }?>>
  <option value="text" <?php if($post_val->ctype=='text'){ echo 'selected="selected"';}?>><?php _e('Text','ddb_wp');?></option>
   <option value="texteditor" <?php if($post_val->ctype=='texteditor'){ echo 'selected="selected"';}?>><?php _e('Text Editor','ddb_wp');?></option>
   <option value="head" <?php if($post_val->ctype=='head'){ echo 'selected="selected"';}?>><?php _e('Text Heading','ddb_wp');?></option>
  <option value="checkbox" <?php if($post_val->ctype=='checkbox'){ echo 'selected="selected"';}?>><?php _e('Checkbox','ddb_wp');?></option>
  <option value="date" <?php if($post_val->ctype=='date'){ echo 'selected="selected"';}?>><?php _e('Date Picker','ddb_wp');?></option>
   <option value="multicheckbox" <?php if($post_val->ctype=='multicheckbox'){ echo 'selected="selected"';}?>><?php _e('Multi Checkbox','ddb_wp');?></option>
  <option value="radio" <?php if($post_val->ctype=='radio'){ echo 'selected="selected"';}?>><?php _e('Radio','ddb_wp');?></option>
  <option value="select" <?php if($post_val->ctype=='select'){ echo 'selected="selected"';}?>><?php _e('Select','ddb_wp');?></option>
  <option value="textarea" <?php if($post_val->ctype=='textarea'){ echo 'selected="selected"';}?>><?php _e('Textarea','ddb_wp');?></option>
   <option value="upload" <?php if($post_val->ctype=='upload'){ echo 'selected="selected"';}?>><?php _e('Upload','ddb_wp');?></option>
   <option value="image_uploader" <?php if($post_val->ctype=='image_uploader'){ echo 'selected="selected"';}?>><?php _e('Multiple image uploader','ddb_wp');?></option>
   
  </select>
<br />    <p><?php _e('Select the type of this custom field.','ddb_wp');?></p>
  </td>
  </tr>
  <tr id="ctype_option_tr_id"  <?php if($post_val->ctype=='select'){?> style="display:block;" <?php }else{?> style="display:none;" <?php }?>>
  <td ><strong><?php _e('Option Values : ','ddb_wp');?></strong></td>
  <td align="left"><input type="text" name="option_values" id="option_values" value="<?php echo $post_val->option_values;?>" size="50" />
   <br />    <span><?php _e('Comma seperated values.','ddb_wp');?></span>
  </td>
  </tr>
  <tr id="admin_title_id"  <?php if($post_val->ctype=='head'){?> style="display:none;" <?php }else{?> style="display:block;" <?php }?>>
  <td width="30%"><strong><?php _e('Field name (for admin only) : ','ddb_wp');?></strong></td>
  <td align="left"><input type="text" name="admin_title" id="admin_title" value="<?php //echo $post_val->admin_title;?>" size="50" />
<br />  <p><?php _e('Name of the field <u>for admin reference only</u>. It won\'t be displayed in the front end.','ddb_wp');?></p>
  </td>
  </tr>
  
  <tr id="admin_label_id"  <?php if($post_val->ctype=='head'){?> style="display:none;" <?php }else{?> style="display:block;" <?php }?>>
  <td ><strong><?php _e('Field label (for back-end): ','ddb_wp');?></strong></td>
  <td align="left"><input type="text" name="clabels" id="clabels" value="<?php echo $post_val->clabels;?>" size="50" />
    <br />    <p><?php _e('The field label for the custom field displayed in the back-end.','ddb_wp');?></p>
  </td>
  </tr>
  
  <tr>
  <td ><strong><?php _e('Field name (for front-end): ','ddb_wp');?></strong></td>
  <td align="left"><input type="text" name="site_title" id="site_title" value="<?php echo $post_val->site_title;?>" size="50" />
<br />    <p><?php _e('The name you provide here will be displayed as the field\'s name (label) in the front-end.','ddb_wp');?></p>
  </td>
  </tr>
  
  <tr id="admin_desc_id"  <?php if($post_val->ctype=='head'){?> style="display:none;" <?php }else{?> style="display:block;" <?php }?>>
  <td ><strong><?php _e('Description: ','ddb_wp');?></strong></td>
  <td align="left"><input type="text" name="admin_desc" id="admin_desc" value="<?php echo $post_val->admin_desc;?>" size="50" />
  <br />    <p><?php _e('Custom field description which will appear in the front-end as well as the backend.','ddb_wp');?></p>
  </td>
  </tr>
  
  <tr>
  <td ><strong><?php _e('HTML variable Name: ','ddb_wp');?></strong></td>
  ...
  <br />    <p><?php _e('The HTML variable name for the custom field. IMPORTANT: It should be a unique name.','ddb_wp');?></p>
  </td>
  </tr>

    
  <tr id="default_value_id"  <?php if($post_val->ctype=='head'){?> style="display:none;" <?php }else{?> style="display:block;" <?php }?>>
  <td ><strong><?php _e('Default value: ','ddb_wp');?></strong></td>
  <td align="left"><input type="text" name="default_value" id="default_value" value="<?php echo $post_val->default_value;?>" size="50" />
    <br />    <p><?php _e('Enter the default value of the custom field.','ddb_wp');?></p>
  </td>
  </tr>
  <tr>
  <td ><strong><?php _e('Position (Display order): ','ddb_wp');?></strong></td>
  <td align="left"><input type="text" name="sort_order" id="sort_order"  value="<?php echo $post_val->sort_order;?>" size="50" />
      <br />    <p><?php _e('This is a numeric value that determines the position of the custom field in the front-end and the back-end. e.g. 5','ddb_wp');?></p>
  </td>
  </tr>
  <tr>
  <td ><strong><?php _e('Activated?: ','ddb_wp');?></strong></td>
  <td align="left">
  <select name="is_active" id="is_active">
  <option value="1" <?php if($post_val->is_active=='1'){ echo 'selected="selected"';}?>><?php _e('Yes','ddb_wp');?></option>
  <option value="0" <?php if($post_val->is_active=='0'){ echo 'selected="selected"';}?>><?php _e('No','ddb_wp');?></option>
  </select>
      <br />    <p><?php _e('This setting activates/de-activates the custom field in the front-end and the back-end.','ddb_wp');?></p>
 </td>
  </tr>
   <tr id="is_require_id"  <?php if($post_val->ctype=='head'){?> style="display:none;" <?php }else{?> style="display:block;" <?php }?>>
  <td ><strong><?php _e('Compulsory?: ','ddb_wp');?></strong></td>
  <td align="left">
  <select name="is_require" id="is_require" <?php if($post_val->is_delete=='1'){?> disabled="disabled" <?php }?>>
  <option value="1" <?php if($post_val->is_require=='1'){ echo 'selected="selected"';}?>><?php _e('Yes','ddb_wp');?></option>
  <option value="0" <?php if($post_val->is_require=='0'){ echo 'selected="selected"';}?>><?php _e('No','ddb_wp');?></option>
  </select>
      <br />    <p><?php _e('Specify whether or not this field is required to be filled in compulsarily by users.','ddb_wp');?></p>
 </td>
  </tr>
  <tr id="show_on_listing_id"  <?php if($post_val->ctype=='head'){?> style="display:none;" <?php }else{?> style="display:block;" <?php }?>>
  <?php /*?><td ><strong><?php _e('Display on Listing Page ? : ','ddb_wp');?></strong></td>
  <td align="left">
  <select name="show_on_listing" id="show_on_listing" <?php if($post_val->is_delete=='1'){?> disabled="disabled" <?php }?>>
  <option value="1" <?php if($post_val->show_on_listing=='1'){ echo 'selected="selected"';}?>><?php _e('Yes','ddb_wp');?></option>
  <option value="0" <?php if($post_val->show_on_listing=='0'){ echo 'selected="selected"';}?>><?php _e('No','ddb_wp');?></option>
  </select>
      <br />    <span><?php _e('Want to display this on Listing page ?','ddb_wp');?></span>
  </td><?php */?>
  </tr>
  <tr id="show_on_detail_id"  <?php if($post_val->ctype=='head'){?> style="display:none;" <?php }else{?> style="display:block;" <?php }?>>
  <td ><strong><?php _e('Show on detail page? : ','ddb_wp');?></strong></td>
  <td align="left">
  <select name="show_on_detail" id="show_on_detail" <?php if($post_val->is_delete=='1'){?> disabled="disabled" <?php }?>>
  <option value="1" <?php if($post_val->show_on_detail=='1'){ echo 'selected="selected"';}?>><?php _e('Yes','ddb_wp');?></option>
  <option value="0" <?php if($post_val->show_on_detail=='0'){ echo 'selected="selected"';}?>><?php _e('No','ddb_wp');?></option>
  </select>
      <br />    <span><?php _e('Specify whether or not this field be shown on the &lsquo;Detail page&rsquo;.','ddb_wp');?></span>
  </td>
  </tr>
    <tr>
  <td >&nbsp;</td>
  <td align="left"><input type="submit" class="button" name="save" id="save" value="<?php _e('Save','ddb_wp');?>" /> <a href="<?php echo site_url();?>/wp-admin/admin.php?page=custom"><input type="button" name="cancel" value="<?php _e('Cancel','ddb_wp');?>" class="button" /></a></td>
  </tr>
</table>
</form>
<script type="text/javascript">
