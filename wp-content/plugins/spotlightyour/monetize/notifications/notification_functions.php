<?php
define('DDBWP_NOTIFICATION_MODULE', __("Manage Notifications",'ddb_wp'));
define('DDBWP_NOTIFICATION_CURRENT_VERSION', '1.0.0');
define('DDBWP_NOTIFICATION_LOG_PATH','http://ddb_wp.com/updates/monetize/notifications/notifications_change_log.txt');
define('DDBWP_NOTIFICATION_ZIP_FOLDER_PATH','http://ddb_wp.com/updates/monetize/notifications/notifications.zip');
define('DDB_NOTIFICATION_FOLDER','notifications');
define('DDB_NOTIFICATION_MODULES_PATH',DDB_MODULES_FOLDER_PATH . DDB_NOTIFICATION_FOLDER.'/');

//----------------------------------------------
     //MODULE AUTO UPDATE START//
//----------------------------------------------
add_action('DDBWP_module_auto_update','DDBWP_module_auto_update_notification_fun');
function DDBWP_module_auto_update_notification_fun()
{
	$curversion = DDBWP_NOTIFICATION_CURRENT_VERSION;
	$liveversion = tmpl_current_framework_version(DDBWP_NOTIFICATION_LOG_PATH);
...
<tr>
<td class="module"><h3><?php echo DDBWP_NOTIFICATION_MODULE;?></h3></td>
</tr>
<tr>
<td>
<form method="post"  name="framework_update" id="framework_update">
<input type="hidden" name="action" value="<?php echo DDB_NOTIFICATION_FOLDER;?>" />
<input type="hidden" name="zip" value="<?php echo DDBWP_NOTIFICATION_ZIP_FOLDER_PATH;?>" />
<input type="hidden" name="log" value="<?php echo DDBWP_NOTIFICATION_LOG_PATH;?>" />
<input type="hidden" name="path" value="<?php echo DDB_NOTIFICATION_MODULES_PATH;?>" />
<?php wp_nonce_field('update-options'); ?>

<?php echo sprintf(__('<h4>A new version of Manage Notifications Module is available.</h4>
<p>This updater will collect a file from the ddb_wp.com server. It will download and extract the files to your current theme&prime;s functions folder. 
...
<br />&rArr; Current Version: %s </p>','ddb_wp'),DDB_NOTIFICATION_MODULES_PATH,$curversion,$liveversion); ?>

<input type="submit" class="button" value="<?php _e('Update','ddb_wp');?>" onclick="document.getElementById('framework_upgrade_process_span_id').style.display=''" />
</form>
</td>
</tr>
...
include_once(DDB_NOTIFICATION_MODULES_PATH . 'notification_options.php');

/////////admin menu settings start////////////////
add_action('DDBWP_admin_menu', 'notification_add_admin_menu');
function notification_add_admin_menu()
{
	add_submenu_page('ddb_wp_wp_admin_menu', DDBWP_NOTIFICATION_MODULE, DDBWP_NOTIFICATION_MODULE, DDBWP_ACCESS_USER, 'notification', 'notification');
}

function notification()
{
	DDB_checkLicence();
	include_once(DDB_NOTIFICATION_MODULES_PATH . 'admin_notification.php');
}
