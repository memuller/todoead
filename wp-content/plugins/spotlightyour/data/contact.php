...
if (empty($action)) {
	...
	<div class='contact-content'>
		<h1 class='contact-title'>The admin hasn't activated this offer.</h1>
		
		<div class='contact-loading' style='display:none'></div>
		<div class='contact-message' style='display:none'></div>
		<form action='#' style='display:none'>
		<p style='padding:20px;'>Send a notification to admin by using the form:</p>
	...
	$message_body = '<strong>'.$name.'</strong>, '.$email.' wants to purchase an offer from '.get_bloginfo( 'wpurl' ).'.
	<br/><br/>  
To activate your offer, you need to upgrade your account by <a href="'.$payment_url.'">clicking here</a>, and inputting your merchant account information, paypal info, or using an external buy button URL.  <a href="'.$payment_url.'">Click here to upgrade your account</a>.
<br/><br/>
Thank you, <br/>
SpotlightYour.com Administration
';
	
	$subject = isset($_POST["subject"]) ? $_POST["subject"] : $subject;
	$message = isset($_POST["message"]) ? $_POST["message"] : $message_body;
...
function smcf_send($name, $email, $subject, $message, $cc) {
	...
	// Build header
	$headers = "From: ".get_bloginfo('name')." <support@".$_SERVER['HTTP_HOST'].">\n";
	if ($cc == 1) {
		$headers .= "Cc: $email\n";
	}
